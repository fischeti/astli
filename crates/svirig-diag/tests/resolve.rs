...
fn find(origins: &Origins, file: FileId, needle: &str) -> Span {
...
fn a_token_written_where_it_is_used_resolves_to_itself() {
    ...
    let diag = Diagnostic::error(CODE, TokenOrigin::written(at), "wrong");
...
fn the_same_complaint_about_the_same_place_is_said_once() {
    ...
    let at = TokenOrigin::written(find(&origins, file, "a"));
    let elsewhere = TokenOrigin::written(find(&origins, file, "b"));
...
fn an_included_file_carries_the_chain_that_reached_it() {
    ...
    let diag = Diagnostic::error(CODE, TokenOrigin::written(find(&origins, f, "logic")), "x");
    let resolved = resolve(&origins, &diag);
