...
//! be answerable without one. `astli-syntax`'s own tests cover the same
...
fn find(origins: &Origins, file: astli_text::SourceId, needle: &str) -> Span {
...
fn line_and_column_count_from_one() {
    ...
    assert_eq!(
        ...
        astli_text::LineCol { line: 1, col: 6 }
    );
}
