...
mod tests {
    ...
    fn a_variable_is_read_from_the_environment_either_way_it_is_written() {
        unsafe { std::env::set_var("ASTLI_TEST_ROOT", "/rtl") };

        let path = Path::new("x.f");
        assert_eq!(
            substitute(path, 1, "$ASTLI_TEST_ROOT/a.sv").unwrap(),
            "/rtl/a.sv"
        );
        assert_eq!(substitute(path, 1, "${ASTLI_TEST_ROOT}x").unwrap(), "/rtlx");
        assert_eq!(substitute(path, 1, "plain.sv").unwrap(), "plain.sv");
    }

    #[test]
    fn a_variable_that_is_not_set_is_an_error_and_not_an_empty_path() {
        let err = substitute(Path::new("x.f"), 7, "$ASTLI_TEST_UNSET/a.sv").unwrap_err();
