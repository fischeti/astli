...
use svirig_text::{Code, Diagnostic, Expansion, Origins, Severity, Span, TokenOrigin};
...
fn a_diagnostic_carries_what_a_renderer_needs() {
    ...
    let at = TokenOrigin::written(find(&origins, file, "`FOO"));
    ...
    assert_eq!(
        origins.line_col(file, diag.at.spelled.start).to_string(),
        "1:1"
    );
}
...
fn a_label_points_somewhere_the_chain_does_not() {
    ...
    let diag = Diagnostic::warning(
        Code("redefined"),
        TokenOrigin::written(call),
        "`FOO` is redefined",
    )
    .label(TokenOrigin::written(def), "the previous definition");

    // A label is always secondary: `at` is what the diagnostic is about.
    assert_eq!(diag.labels.len(), 1);
    assert_eq!(diag.labels[0].at.spelled.file, header);
    assert_eq!(diag.at.spelled.file, top);
    assert_eq!(origins.slice(diag.labels[0].at.spelled), "`define FOO 1");
}
...
fn a_diagnostic_about_an_expanded_token_keeps_both_ends() {
    ...
    let at = TokenOrigin {
        spelled: find(&origins, header, "nonexistent_t"),
        from: Some(expansion),
    };
    ...
    assert_eq!(diag.at.spelled.file, header);
    ...
    let chain: Vec<_> = origins.trace(diag.at).collect();
