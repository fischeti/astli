...
use astli_text::{SourceId, Span};

use astli_syntax::{SyntaxKind, Token};
...
impl TokenId {
    ...
    pub fn span(self) -> TokenSpan {
        TokenSpan::new(self.src_id, self.index, self.index + 1)
    }
    ...
    pub fn bytes(self, tokens: &[Token]) -> Span {
        ...
        Span::new(self.src_id, token.start, token.end)
    }
}
...
impl TokenSpan {
    ...
    pub fn with(self, range: Range<u32>) -> TokenSpan {
        TokenSpan::new(self.src_id, range.start, range.end)
    }
    ...
    pub fn bytes(self, tokens: &[Token]) -> Span {
        ...
        if self.is_empty() {
            return Span::point(self.src_id, tokens[at].start);
        }
        Span::new(
            self.src_id,
            tokens[at].start,
