...
fn main() -> ExitCode {
    ...
    if !show_origins {
        print!("{}", render(session.origins(), &tokens));
    ...
    for token in tokens.iter().filter(|token| token.origin.from.is_some()) {
        ...
        let at = match session.origins().path(spelled.file) {
            Some(path) => format!(
                "{}:{}",
                path.display(),
                session.origins().line_col(spelled.file, spelled.start)
            ),
            // Pasted or stringified: the bytes are in no file.
            None => "<synthesised>".to_string(),
        };
        let through: Vec<_> = session
            .origins()
            .trace(token.origin)
            .map(|expansion| session.origins().slice(expansion.name))
            .collect();
        ...
        println!(
            ...
            at,
            format!("{:?}", token.kind),
            format!("{:?}", session.origins().slice(spelled)),
            through.join(" < ")
        );
    }
