...
| D4 | Column alignment is a post-pass over emitted lines | lowRISC requires aligned connections and encourages aligned declarations. Runs of same-shaped siblings align, broken by blank lines only: a comment line annotates a group, a blank line ends one. A line that padding would take past the width is left unpadded. Precedent: gofmt's `tabwriter`. |
